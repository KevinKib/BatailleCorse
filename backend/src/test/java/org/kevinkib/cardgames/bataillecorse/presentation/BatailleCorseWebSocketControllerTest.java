...
class BatailleCorseWebSocketControllerTest {
    ...
    @BeforeEach
    void setUp() {
        sessionService = new SessionService(new InMemorySessionRepository(java.time.Clock.systemUTC()), new org.kevinkib.cardgames.sessionmanagement.session.application.GameFactories(java.util.List.of(new org.kevinkib.cardgames.bataillecorse.domain.BatailleCorseFactory())));
    ...
    class RematchTest {
        ...
        void givenSoloBothSeatsRequest_whenSecondRematch_thenBroadcastsStarted() {
            var game = sessionService.createGame("bataille-corse", 2, org.kevinkib.cardgames.sessionmanagement.session.domain.GameMode.SOLO);
            String gameId = game.getId().uuid().toString();
            SessionToken token0 = sessionService.loadTokenByPlayerId(game.getId(), new PlayerId(0));
            SessionToken token1 = sessionService.loadTokenByPlayerId(game.getId(), new PlayerId(1));
        ...
        void givenMultiplayerSingleRequest_whenRematch_thenBroadcastsPending() {
            var game = sessionService.createGame("bataille-corse", 2, org.kevinkib.cardgames.sessionmanagement.session.domain.GameMode.MULTIPLAYER);
