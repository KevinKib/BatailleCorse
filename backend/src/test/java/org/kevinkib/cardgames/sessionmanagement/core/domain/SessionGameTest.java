...
class SessionGameTest {
    ...
    class RematchTest {
        ...
        @Test
        void givenRoomWithUnclaimedSeats_whenRematch_thenOnlyClaimedSeatsCount() {
            SessionGame session = SessionGame.create(GameId.generate(), 6, "bullshit"); // room of 6
            session.claimHost("Alice");           // seat 0
            session.claimNextFreeSeat("Bob");     // seat 1
            session.claimNextFreeSeat("Cara");    // seat 2 — 3 joined; seats 3-5 never claimed
            session.requestRematch(new PlayerId(0));
            session.requestRematch(new PlayerId(1));

            assertThat(session.rematchStayingCount(), is(3));  // unclaimed seats excluded, not 6
            assertThat(session.rematchReadyCount(), is(2));
            assertThat(session.isRematchReady(), is(false));   // seat 2 hasn't asked yet

            session.requestRematch(new PlayerId(2));
            assertThat(session.isRematchReady(), is(true));    // all three who joined have asked
        }
    }
}
